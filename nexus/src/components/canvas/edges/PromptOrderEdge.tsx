...
export const PromptOrderEdge = memo(function PromptOrderEdge({
  ...
  const handleOrderSelect = useCallback((n: number) => {
    useGraphStore.getState().setEdgePromptOrder(id, n)
    setEdges(edges => edges.map(e => e.id === id ? { ...e, data: { ...e.data, promptOrder: n } } : e))
  ...
  return (
    <>
      ...
      <path d={edgePath} fill="none" className="react-flow__edge-path" style={{ strokeWidth: 2.5, strokeOpacity: isHighlighted ? 0.8 : 0.1, transition: 'stroke 0.3s, stroke-opacity 0.3s', stroke: selected ? color : `url(#${gradientId})` }} />
      ...
      <EdgeLabelRenderer>
        ...
        >
          <button
            ...
            className={`flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold shadow-md border transition-shadow cursor-pointer ${
          ...
          {showDropdown && (
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-xl overflow-hidden min-w-[40px]" style={{ zIndex: 9999 }} onMouseDown={e => e.stopPropagation()}>
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} onClick={(e) => { e.stopPropagation(); handleOrderSelect(n) }} onMouseDown={e => e.stopPropagation()} className={`w-full px-3 py-1.5 text-xs text-center hover:bg-gray-100 dark:hover:bg-gray-700 ${order === n ? 'font-bold text-emerald-600' : 'text-gray-700 dark:text-gray-200'}`}>
                  {n}
                </button>
              ))}
            </div>
