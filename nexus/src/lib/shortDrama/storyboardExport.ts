...
export async function exportStoryboardPdf(draft: ShortDramaDraftV2): Promise<Uint8Array> {
  ...
  titlePage.drawText(draft.title || 'Storyboard', { x: 50, y: 780, size: 24, font })
  titlePage.drawText(draft.logline || '', { x: 50, y: 750, size: 12, font, maxWidth: 495 })
  ...
  for (const shot of draft.shots || []) {
    ...
    page.drawText(shot.title || 'Shot', { x: 50, y, size: 16, font })
    y -= 25

    if (shot.beat) {
      page.drawText(`Beat: ${shot.beat}`, { x: 50, y, size: 10, font, color: rgb(0.4, 0.4, 0.4) })
      y -= 20
    }

    page.drawText('Start Frame:', { x: 50, y, size: 10, font, color: rgb(0, 0.4, 0.8) })
    y -= 15
    const startText = shot.frames?.start?.prompt || ''
    if (startText) {
    ...
    y -= 15
    const endText = shot.frames?.end?.prompt || ''
    if (endText) {
    ...
    if (shot.videoPrompt) {
      ...
      y -= 15
      const lines = wrapText(shot.videoPrompt, 80)
      for (const line of lines) {
        page.drawText(line, { x: 50, y, size: 9, font })
        y -= 13
      }
